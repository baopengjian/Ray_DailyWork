...
public class CalendarReminderUtils {
    ...
    public static List<Map<String, String>> query(Context context) {
        ...
        try {
            ...
            if (eventCursor.getCount() > 0 && accountId > 0) {
                //遍历所有事件，找到title跟需要查询的title一样的项
                for (eventCursor.moveToFirst(); !eventCursor.isAfterLast(); eventCursor.moveToNext()) {
                    int calId = eventCursor.getInt(eventCursor.getColumnIndex(CalendarContract.Events.CALENDAR_ID));
                    if (accountId == calId) {
                        Map<String, String> map = new HashMap<>();
                        map.put(CalendarContract.Events.TITLE, eventCursor.getString(eventCursor.getColumnIndex(CalendarContract.Events.TITLE)));
                        map.put(CalendarContract.Events.DESCRIPTION, eventCursor.getString(eventCursor.getColumnIndex(CalendarContract.Events.DESCRIPTION)));
                        list.add(map);
                    }
                }
            }
