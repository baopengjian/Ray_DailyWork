...
public class SwitchButton extends View implements Checkable {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mTextOn = typedArray.getText(R.styleable.SwitchButton_sb_text_on);
        mTextOff = typedArray.getText(R.styleable.SwitchButton_sb_text_off);
        mTextOnColor = optColor(typedArray, R.styleable.SwitchButton_sb_text_on_color, 0Xff51d367);
        mTextOffColor = optColor(typedArray, R.styleable.SwitchButton_sb_text_off_color, Color.GRAY);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        drawArc(canvas, left, top, left + 2 * viewRadius, top + 2 * viewRadius,
                90, 180, paint);
        ...
        drawButtonText(canvas, viewState.buttonX, centerY);
    }
     ...
     * @param y      centerY
     */
    private void drawButtonText(Canvas canvas, float x, float y) {
        float interVal = buttonRadius / 2.00F;
        RectF rectF = new RectF(x - interVal, y - interVal, x + interVal, y + interVal);
        if (isChecked() && !TextUtils.isEmpty(mTextOn)) {
            textPaint.setColor(mTextOnColor);
            DrawTextUtility.drawText(mTextOn.toString(), rectF, canvas, sp2px(14f), Gravity.CENTER, textPaint);
        } else if (!isChecked() && !TextUtils.isEmpty(mTextOff)) {
            textPaint.setColor(mTextOffColor);
            DrawTextUtility.drawText(mTextOff.toString(), rectF, canvas, sp2px(14f), Gravity.CENTER, textPaint);
        }
    }
    ...
    private int mTextOnColor, mTextOffColor;
    /**
     * 是否选中
     */
